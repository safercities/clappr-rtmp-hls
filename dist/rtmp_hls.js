...
var RtmpHls = (function (_Playback) {
  function RtmpHls(options) {
    ...
    this.RTMP.once(Events.PLAYBACK_READY, function () {
      return _this2.bootstrap();
    });
    this.HLS.once(Events.PLAYBACK_READY, function () {
      return _this2.bootstrap();
    });
  }
  ...
  _createClass(RtmpHls, {
    ...
    switchPlayer: {
      value: function switchPlayer(newPlayer) {
    ...
    addFulltimeListeners: {
      value: function addFulltimeListeners() {
        var _this = this;

        //Every x seconds the HLS player when paused will refresh the m3u8 and latest TS and update duration. How convenient
        this.HLS.on(Events.PLAYBACK_TIMEUPDATE, function (hlsPos, hlsDur) {
          if (!_this.isPlaying()) return;

          _this.duration = hlsDur;
          var pos = _this.currentPlayer == "HLS" ? hlsPos : hlsDur; //Only use the HLS position if we're using the HLS player
          _this.trigger(Events.PLAYBACK_TIMEUPDATE, pos, _this.duration, _this.name);
        });
      }
    },
    addRtmpListeners: {
      value: function addRtmpListeners() {
        ...
        this.RTMP.on(Events.PLAYBACK_BUFFERFULL, function () {
          return _this.setPlaybackState("PLAYBACK_BUFFERFULL");
        });
        this.RTMP.on(Events.PLAYBACK_BUFFERING, function () {
          return _this.setPlaybackState("PLAYBACK_BUFFERING");
        });
        this.RTMP.on(Events.PLAYBACK_PLAY, function () {
          return _this.setPlaybackState("PLAYING");
        });
      }
    },
    removeRtmpListeners: {
    ...
    addHlsListeners: {
      value: function addHlsListeners() {
        ...
        //Bubble RTMP live events back up
        this.HLS.on(Events.PLAYBACK_BUFFERFULL, function () {
          return _this.setPlaybackState("PLAYING");
        });
      }
    },
    ...
    play: {
      value: function play() {
        ...
        if (this.currentState == "IDLE") {
          ...
          this.HLS.once(Events.PLAYBACK_BUFFERFULL, function () {
            return _this.HLS.pause();
          });
          this.HLS.play();
          this.RTMP.play();
        }
...
},{"./jst":2,"events":"events","lodash.assign":3,"mediator":"mediator","playback":"playback","player_info":"player_info"}],2:[function(require,module,exports){
